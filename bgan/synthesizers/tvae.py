...
This module implements the Tabular Variational Autoencoder (TVAE) for tabular data synthesis.
TVAE is based on the implementation and concepts from the CTGAN project
(https://github.com/sdv-dev/CTGAN), which introduces a variational autoencoder approach
for synthesizing tabular data with support for both continuous and discrete columns.

Key features:
- Variational Autoencoder architecture with encoder and decoder networks
- Support for mixed data types (continuous and categorical columns)
- KL divergence regularization for the latent space
- Reconstruction loss with learned variance per feature

This code is inspired by and adapted from CTGAN (MIT License):
https://github.com/sdv-dev/CTGAN/blob/master/LICENSE

MIT License applies to portions derived from CTGAN:
Copyright (c) 2019 Synthetic Data Vault (SDV)
...
- CTGAN codebase (MIT License): https://github.com/sdv-dev/CTGAN
  Portions of this code are adapted from CTGAN, with modifications as needed
  for the synthesis pipeline. MIT License applies to those portions.
"""
